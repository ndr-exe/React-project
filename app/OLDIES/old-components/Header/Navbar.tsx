...
export default async function Navbar({ dict }: { dict: DictType }) {
  ...
  return (
    <nav className="w-full h-full flex">
      <h1 className="flex items-center text-2xl pl-8 dark:text-gray-100">
        fast <span className="text-orange-600">LAG</span>.
      </h1>
        ...
        <li className="text-lg pr-6 ">
          {session ? (
            <a
              ...
              className="hover:text-orange-600 font-bold dark:text-gray-100"
            >
              {dict.options.logout}
            </a>
          ) : (
